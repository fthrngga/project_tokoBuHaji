                ...
                <main className="flex-1 pb-32">
                    ...
                    <div className="bg-[#0d1e2e]/50 border-b border-[#577399]/20 relative overflow-hidden">
                        ...
                        <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-16 lg:py-24 relative z-10">
                            ...
                            <nav className="flex items-center space-x-2 text-sm text-[#bdd5ea]/60 mb-6 font-medium">
                                ...
                                <span>/</span>
                                <span className="text-white">Semua Produk</span>
                            </nav>
                            ...
                            {(parentCategories && parentCategories.length > 0) && (
                                <div className="mt-12 flex items-center gap-3 overflow-x-auto pb-4 hide-scrollbar animate-in fade-in duration-700 delay-300">
                                    ...
                                    <Link 
                                        href={`/kategori`}
                                        className="shrink-0 px-6 py-2.5 rounded-full text-sm font-medium border transition-all bg-[#FE5F55] text-white border-[#FE5F55] shadow-[0_4px_15px_rgba(254,95,85,0.4)] hover:scale-105"
                                    >
                                        Semua
                                    </Link>
                                    ...
                                    {parentCategories.map(sub => (
                                        <Link 
                                            ...
                                            className="shrink-0 px-6 py-2.5 rounded-full text-sm font-medium border transition-all bg-[#0d1e2e] text-[#bdd5ea] border-[#577399]/30 hover:border-[#FE5F55]/50 hover:bg-[#577399]/10 hover:text-white"
                                        >
                                            {sub.name}
                                        </Link>
                                    ))}
                    ...
                    <div className="container mx-auto px-4 sm:px-6 lg:px-8 mt-12 lg:mt-20">
                        {products.data.length === 0 ? (
                            <div className="text-center py-20 px-4 border border-[#577399]/10 rounded-3xl bg-[#0d1e2e]/30">
                                ...
                                <h3 className="text-2xl font-bold text-white mb-3 tracking-tight">Koleksi Kosong</h3>
                                <p className="text-[#bdd5ea]/60 max-w-md mx-auto">Kami sedang menyiapkan produk terbaik untuk Anda. Silakan kembali lagi nanti.</p>
                            </div>
                        ) : (
                        ...
                        {products.links && products.links.length > 3 && (
                            <div className="mt-20 flex justify-center">
                                <nav className="flex items-center gap-1 p-1 bg-[#0d1e2e]/50 border border-[#577399]/20 rounded-full backdrop-blur-sm">
                                    {products.links.map((link, idx) => {
                                        if (link.url === null) {
                                            return (
                                                <span 
                                                    key={idx} 
                                                    className="px-4 py-2 text-sm text-[#bdd5ea]/40 font-medium"
                                        ...
                                        return (
                                            <Link
                                                ...
                                                className={`px-4 py-2 text-sm font-medium rounded-full transition-all duration-300 ${
                                                    link.active
                                                        ? 'bg-[#FE5F55] text-white shadow-md'
                                                        : 'text-[#bdd5ea] hover:bg-[#577399]/20 hover:text-white'
                                                }`}
                                                dangerouslySetInnerHTML={{ __html: link.label }}
                                            />
                                        );
